...
- **Every dropdown becomes selectable option tiles.** Property type = 5 tiles with imagery (owned-photo thumbnails, supplied glyph-mosaic thumbs if Razim delivers them — add a row to the §3 manifest — or neutral line-art type icons; **no stock photography**, the ban stands); tier/market, condition, brand, ground lease, F&B = tile/segment groups. Tiles are real radio groups (labels, arrow-key navigation, 44px, checked = accent ring + chip).
