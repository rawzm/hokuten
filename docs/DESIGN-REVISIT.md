...
**Theme B color bias (Razim, 2026-08-08).** Artwork destined for Hokuten Blue contexts biases its color translation toward: **Hokuten blue / dusty blue → pale ivory → warm sand → muted salmon → olive → deep navy/burgundy** — sophisticated, never hacker/code-art. Per-theme artwork variants are therefore allowed: the manifest gains a theme column, and a placement may carry one shared piece or a gold/blue pair. Record this ramp in ref 01 beside the Theme B palette. (Delivered piece A3 below already sits in this palette — it is the natural Theme B hero candidate.)

**First batch DELIVERED 2026-08-08** — masters in `Ref/artwork/` (rename on intake to stable kebab names; originals untouched):

| # | File (time-stamped) | Shape | Subject | Proposed placement (art-direct on the preview page; Razim eyeballs) |
|---|---|---|---|---|
| A1 | `…03_19_00 PM` | 1448×1086 (4:3) | Holiday Inn Express entrance at dusk — the HIE Brooklyn closing's subject, warm glow on dark | **Menu panel** (3:4 portrait crop) · Method-chapter candidate (dark ground) · select-service tile crop |
| A2 | `…03_19_03 PM` | 1672×941 (16:9) | Resort tower + palms + pool, vivid blues/greens | Hero candidate (Theme G) · resort/boutique tile source |
| A3 | `…03_22_12 PM` | 1672×941 (16:9) | Full-service block at sunset — dusty blue/salmon/sand sky | **Hero band, Theme B** (matches the palette bias) · Method-chapter candidate |
| A4 | `…03_23_44 PM` | 1536×1024 (3:2) | Branded full-service tower, warm sand facade | Full-service tile source · closings-section accent |
| A5 | `…03_25_21 PM` | 1942×809 (~2.4:1) | Beachfront aerial — ocean, beach, white towers | **Hero band, Theme G** (the widest piece) · listing-placeholder crop source |

Note for the record: several pieces carry visible third-party signage (HIE, Marriott) inside the artwork — same nominative posture and counsel flag as the brands marquee; internal-only until the launch gate. A1 depicts an actual track-record property, which is the preferred pattern for future pieces.
...
1. **Artwork intake manifest.** Maintain it in `content/artwork.ts` — a typed registry mapping placement → asset path + alt + status, so a delivered file is a data edit, not a refactor. Until a piece lands, its placement renders the current interim art (the existing static ASCII frame or a designed surface) and carries `blocked: awaiting-artwork` in `docs/PLACEHOLDERS.md`. Pieces to request from Razim:
   ...
   | Piece | Placement | Target shape | Status |
   |---|---|---|---|
   | Hero band, Theme G | §4.2 | ~2:1–21:9 landscape, ≥2560px wide ideal | **candidate delivered (A5**, 1942w — ship it; request a ≥2560w re-render only if it softens at 1440+**)** |
   | Hero band, Theme B | §4.2 | same | **candidate delivered (A3)** |
   | Menu panel | §4.3 overlay left third | portrait ~3:4, ≥1200px wide | **crop from A1**; a native-portrait piece welcome later |
   | Method chapter | §4.7, dark ground | ~4:3 or square, ≥1600px | **A1 or A3** (both read on `--dark`) |
   | Listing placeholder | §4.5 ticket header, no-photo listings | ~3:2, ≥1200px | **crop from A5** |
   | Property-type tiles ×5 | §4.6 | **1:1 square, ≥800×800 each** | A1/A2/A4 crops cover select-service / resort / full-service; **request limited-service + extended-stay squares from Razim** |
   | Market-tier panels ×4 | §4.6 | **5:2 wide panel, ~1600×640 each** | optional — text-tile fallback is fine; request only if Razim wants imagery here |
   | (Optional) BOV/doors accent | §4.9 side panel | flexible | only if Razim wants one |

   Drop locations for masters (all production-approved by the D1 exception): `Ref/artwork/` (scene pieces), `Ref/hotel-brands/` (brand chips), `Ref/site/` (identity + awards). Executor optimizes copies into `site/public/art/` and `site/public/logos/`.
...
7. **Brand chips — DELIVERED** (16 files in `Ref/hotel-brands/`, each a glossy 3D glass squircle chip per flag — Marriott, Best Western, Ritz-Carlton, etc. — centred on near-white, 1672×941). Prep with sharp, scripted and repeatable: detect the chip's content bounding box against the near-white ground (keep the chip's own gloss edge and soft shadow — that IS the 3D dimension; do not flatten it), crop with uniform margin, knock the surround to transparency, normalize every chip to the same square canvas so the marquee row sits optically even. Uniform render height ~44–52px desktop / ~36px mobile (these are dimensional objects — they carry more presence than flat marks, so slightly larger than the old 28px rule; update ref 04). Output `site/public/logos/<flag>.png` @2x + AVIF. `docs/design/LOGO-MANIFEST.md` gets one row per chip (source: Razim-supplied rendering, 2026-08-08). The shipped set includes luxury flags (Ritz-Carlton) — update ref 06's `#brands` register row wording so the coverage claim matches the set actually rendered ("economy through luxury"), still framed as "flags we transact across", never partners/clients.
8. **Option-tile shape contract (§4.6)** — locked so Razim can produce imagery to fit: **property type = 1:1 squares** (five across on desktop, image fills the tile, label bar is UI — keep the subject centred, safe from edge crops; ≥800×800). **Market tier = 5:2 wide panels** (~1600×640, stacked full-width rows; muted, label overlaid by UI). Binary/ternary options (condition, brand affiliation, ground lease, F&B) stay **text segment chips — no imagery**; pictures on yes/no choices read as noise. All tile imagery goes through the 北天 treatment for cohesion.
...
- **Every dropdown becomes selectable option tiles**, to the shape contract in §3.8: property type = 1:1 image squares (A1/A2/A4 crops + the two squares requested from Razim; interim = designed typographic tiles, never stock); market tier = 5:2 wide panels (or text tiles until imagery lands); condition / brand / ground lease / F&B = text segment chips. Tiles are real radio groups (labels, arrow-key navigation, 44px, checked = accent ring + chip).
