...
|---|---|---|
| `Ref/HOYskIPaMAEwBLK.jpeg` + `Ref/HOYsosgaYAAt2xu.jpeg` (runcycle.com hero) | Full-bleed **colored halftone/ASCII art band**: classical subject rendered entirely in tiny glyphs **with the photograph's colors preserved** — blues stay blue, stone stays warm. Art band sits under a clean white nav, is **truncated** (~55–60vh, not full-screen), headline row sits BELOW the art (large serif display left, small sans sub + two pill CTAs right), and a **logo bar** closes the first viewport. | This is the hero anatomy AND the platform art treatment. |
...
5. **D5 — 北天 chromatic sieve, platform-wide.** The site's image treatment becomes **color-preserving ASCII art built from tiny 北天 glyphs** (runcycle translation, ours via the kanji). Plus a **北天 background-motif layer** used like the Method chapter's orbital arcs: large, quiet kanji accents on menu, form sections, and dark chapters. Spec in §3.
...
## 3. Art pipeline v2 — the 北天 chromatic sieve

Extend `site/scripts/ascii-gen.ts` (build-time; the existing mono pipeline, seam row, and budgets stay for reference):

1. **Color mode.** Per-cell color sampling from the source photo, quantized to a ≤32-color palette per artwork (palette-indexed cells keep JSON small). The glyph carries the sampled color; the ground stays the theme surface. This is the runcycle look with our glyphs.
2. **北天 as the base glyph set.** The ramp becomes tiny CJK glyphs — 北 and 天 dominant, ・ and space for air, optionally ホクテン mid-tones. **All CJK = uniform double-width cells**, so use a square-cell grid (charAspect ≈ 1.0) — an all-wide charset eliminates the half-cell shear problem the mono pipeline had to work around. Cell size small enough that the image reads first and the kanji reveal on approach (~8–12px rendered).
3. **Outputs**: hero art (per §4.2, both themes; ambient loop may drop to 12–16 frames to hold budget), menu-overlay panel art, `#method` chapter art (replaces the engraving — Razim: the building drawing is ugly; retire `HotelEngraving` from the page), and optional card-photo treatment. Sources: **owned track-record photos only** (Brooklyn, Carte, Renaissance, etc.).
4. **`<KanjiAccent>` component**: the reusable 北天 background motif — huge outlined/low-opacity 北/天 glyphs (SVG paths, reuse the hanko glyph geometry, never `<text>`), placed like OrbitalArcs: absolute, aria-hidden, pointer-events-none, ≤8% opacity dark / ≤6% light, one per section max. Use in: menu overlay, calculator, BOV, and wherever a side accent is needed. It joins the motif system in ref 01.
5. **Budgets** (update ref 05): per-theme hero color JSON ≤ 400KB gzip; static first-frame fallback as an optimized raster/SVG ≤ 250KB, which is also the LCP-safe layer the canvas hydrates over; loop paused off-screen; reduced-motion/data-saver/mobile = static frame. Keep the seam row: `THE HOKUTEN GROUP` still resolves in the art's lower third.
...
- Row 2: **full-bleed chromatic-sieve art band, ~55–60svh**, edge to edge. Art carries its own colors in both themes; theme governs chrome. Seam row intact. Shimmer + ambient loop live here under the existing HERO_BUDGET discipline.
...
Replace the engraving with a chromatic-sieve render of an owned hotel photo (or a substantially better engraving if the sieve fights the dark ground — art-direct on the preview page, Razim eyeballs). Keep stepper, reach stats, two-tone paragraph, hanko stamp ②. Compress to fit-viewport.
