...
| FRED key status | **PROVISIONED 2026-08-07**: `FRED_API_KEY` is set on the `hokuten` Vercel project in Production, Preview, and Development (same key the kwc site + a100arms share; value in Vercel only, never in the repo or docs). For local dev run `vercel env pull site/.env.local --scope hokuten1`. Gotcha from the kwc team: changing the key in the dashboard does nothing until the next deploy |
