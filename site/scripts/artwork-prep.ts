...
async function buildContactSheet(
  ...
  for (const p of placed) {
    ...
    labels += `<text x="${p.x}" y="${ly + 17}" font-family="monospace" font-size="11" fill="#6E6862">${escapeXml(p.it.slug)} · ${p.it.variant} · crop=${escapeXml(describeCrop(p.it.cropMode))}</text>`;
  }

  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${sheetW}" height="${sheetH}">
    <rect width="100%" height="100%" fill="#FBF9F3"/>
    <text x="${PAD}" y="36" font-family="monospace" font-size="20" font-weight="bold" fill="#1A1C1F">HOKUTEN 「北天」 artwork — contact sheet</text>
    ${images}
    ${labels}
  </svg>`;
